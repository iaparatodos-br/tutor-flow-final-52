...
**Handler `invoice.paid` (linha 300-335)**: [CORREÇÃO v2.4 - Gap 125] **SUBSTITUIR handler completo** (linhas 300-335). O código abaixo SUBSTITUI inteiramente o handler existente, NÃO é uma adição incremental:
...
| **[Gap 115] `payment_intent.succeeded` handler (linha 453) usa `.single()` — mesmo problema do Gap 103** | **Alta** | **Alto** | **O handler ATUAL em produção busca `existingPI` com `.single()` (linhas 453-457). Se um payment intent sucede para um PI que NÃO está no nosso banco (ex: PI criado diretamente no Stripe Dashboard, ou PI de Stripe Invoice que não registramos por `stripe_payment_intent_id`), `.single()` com 0 rows lança erro → 500 → retries infinitos do Stripe por 3 dias. Gap 103 documentou o mesmo problema para `invoice.paid`/`invoice.payment_succeeded` mas OMITIU `payment_intent.succeeded`. FIX: trocar `.single()` por `.maybeSingle()` e adicionar null guard (se null → log warning + break sem erro).** |
| **[Gap 116] `shouldCharge` para faturas prepaid PAGAS não é desativado — risco de double-charging** | **Baixa** | **Alto** | **Na seção 5.4 (`process-cancellation`), o loop de void (linhas 1122-1179 do plano) só define `shouldCharge = false` para faturas com status `'pendente'` ou `'open'`. Se a fatura prepaid já está `'paga'`, o loop NÃO altera `shouldCharge`. Resultado: o aluno perde o valor prepaid (sem refund automático) E recebe uma cobrança adicional de cancelamento. Isso é dupla penalização. FIX: Quando QUALQUER `invoice_classes` com `item_type = 'prepaid_class'` existir para a aula cancelada, definir `shouldCharge = false` INDEPENDENTE do status da fatura. Rationale: o aluno já comprometeu financeiramente via prepaid; cobranças adicionais de cancelamento são excessivas. O professor pode cobrar manualmente se necessário.** |
| **[Gap 117] Agenda.tsx não verifica campo `success` da resposta de `process-class-billing`** | **Média** | **Médio** | **O código proposto em 4.2.1 (linha 332) verifica `billingResult.charge_timing` mas NÃO verifica `billingResult.success` (introduzido pelo Gap 97). Se `process-class-billing` retorna `{ success: false, error: 'Mensagem de erro' }` com um `charge_timing` não reconhecido, o erro é silenciosamente ignorado. FIX: Adicionar verificação prioritária de `billingResult.success === false` com toast do `billingResult.error` ANTES de checar `charge_timing`.** |
| **[Gap 118] Sem rollback se Stripe Invoice é criada/finalizada mas DB insert falha** | **Baixa** | **Alto** | **O passo 3c da seção 5.1 detalha: (iv) criar Invoice Stripe, (v) finalizar, (vi) inserir no DB `invoices`, (vii) inserir no DB `invoice_classes`. Gap 47 trata rollback quando STRIPE falha. Mas se Stripe sucede nos passos iv-v e o DB falha no passo vi, a Invoice Stripe existe finalizada e é pagável, mas nosso sistema não tem registro. O webhook `invoice.paid` faria skip ("not found in local database" per Gap 75). Resultado: aluno paga uma fatura "fantasma" que nunca é reconciliada. FIX: Wrapping steps vi-vii em try/catch; se DB insert falhar, chamar `stripe.invoices.voidInvoice(stripeInvoice.id, { stripeAccount })` antes de retornar erro.** |
| **[Gap 119] Padrão de resposta de erro inconsistente entre `process-cancellation` e `process-class-billing`** | **Baixa** | **Médio** | **`process-class-billing` usa HTTP 200 + `success: false` para erros de business logic (Gap 97, padrão do projeto). `process-cancellation` (linha 495-498) usa HTTP 500 para TODOS os erros, incluindo erros de validação ("Aula não encontrada", "Você não tem permissão"). O frontend (`CancellationModal.tsx`) pode não extrair mensagens user-friendly de respostas HTTP 500. A memory do projeto (`error-handling-user-friendly-messages`) recomenda HTTP 200 + `success: false`. FIX: Documentar como inconsistência conhecida para resolver na Fase 5, OU alinhar `process-cancellation` ao padrão do projeto.** |
| **[Gap 120] Agenda.tsx precisa importar namespace i18n `billing` para toasts de cobrança** | **Baixa** | **Baixo** | **O código proposto em 4.2.1 usa `t('chargeTiming.prepaidInvoiceCreated')` e `t('chargeTiming.stripeNotReady')`. Essas chaves estão definidas em `billing.json` (seção 6.1-6.2). Porém, `Agenda.tsx` provavelmente usa `useTranslation('classes')` ou similar. Sem adicionar 'billing' ao array de namespaces, as chaves retornarão a key literal em vez do texto traduzido. FIX: Garantir que Agenda.tsx importa o namespace `billing`: `const { t } = useTranslation(['classes', 'billing'])` ou usar `t('billing:chargeTiming.prepaidInvoiceCreated')`.** |
| **[Gap 121] Resposta de `process-class-billing` não detalha participantes skipados por mensalidade** | **Baixa** | **Baixo** | **Gap 76 define que participantes com mensalidade ativa são skipados no billing pré-pago. A resposta tem `skipped: number` (agregado) mas não identifica QUEM foi skipado nem o motivo. Professor com aula em grupo (4 alunos: 2 assinantes, 2 não) vê `processed: 2, skipped: 2` sem saber quem são os skipados. FIX: Adicionar campo opcional `skipped_details?: Array<{ participant_id: string, student_name: string, reason: 'active_subscription' \| 'experimental' \| 'already_billed' }>` na interface `ProcessClassBillingResponse`.** |
| **[Gap 122] `process-class-billing` não valida preço e status do serviço server-side** | **Média** | **Médio** | **O passo 3a-bis busca dados de `class_services` para cada aula, mas não valida que `is_active = true` e `price > 0`. Se um serviço for desativado ou tiver preço R$0 entre a criação da aula e o processamento do billing, a função criaria uma Invoice Stripe com valor R$0 (ou item com amount 0). Stripe rejeita items com amount 0 em muitos cenários. FIX: Adicionar validação no passo 3a-bis: `WHERE is_active = true AND price > 0`. Se serviço inválido, logar warning e skippar a aula.** |
...
- [ ] [v2.4] **CRÍTICO PRÉ-DEPLOY**: Atualizar handler `payment_intent.succeeded` EXISTENTE de `.single()` para `.maybeSingle()` com null guard (Gap 115)
- [ ] [v2.4] Verificar que `process-cancellation` define `shouldCharge = false` para QUALQUER fatura prepaid (independente de status pago/pendente) (Gap 116)
- [ ] [v2.4] Verificar que Agenda.tsx verifica `billingResult.success === false` antes de checar `charge_timing` (Gap 117)
- [ ] [v2.4] **CRÍTICO**: Verificar que `process-class-billing` faz void da Stripe Invoice se DB insert (passos vi-vii) falhar após finalização (Gap 118)
- [ ] [v2.4] Documentar inconsistência: `process-cancellation` usa HTTP 500, `process-class-billing` usa HTTP 200 + success:false (Gap 119)
- [ ] [v2.4] Verificar que Agenda.tsx importa namespace i18n `billing` para toasts de cobrança (Gap 120)
- [ ] [v2.4] Verificar que resposta de `process-class-billing` inclui `skipped_details` para participantes skipados por mensalidade (Gap 121)
- [ ] [v2.4] Verificar que `process-class-billing` valida `is_active = true AND price > 0` do serviço server-side (Gap 122)
...
| # | Gap Identificado | Resolução |
|---|------------------|-----------|
| 115 | `payment_intent.succeeded` handler (linha 453) usa `.single()` — mesmo problema do Gap 103 | **CRÍTICO PRÉ-DEPLOY**: Trocar `.single()` por `.maybeSingle()` com null guard. Se não encontrar invoice, log warning e break (sem erro 500). Omitido pelo Gap 103 que cobriu apenas `invoice.paid`/`invoice.payment_succeeded`. |
| 116 | `shouldCharge` para faturas prepaid PAGAS não é desativado | Na lógica de void do `process-cancellation`, `shouldCharge = false` só é definido para faturas `'pendente'`/`'open'`. Se fatura prepaid está `'paga'`, `shouldCharge` mantém valor original → aluno perde prepaid + recebe cobrança de cancelamento (dupla penalização). FIX: definir `shouldCharge = false` quando QUALQUER `invoice_classes` com `item_type = 'prepaid_class'` existir, independente do status da fatura. |
| 117 | Agenda.tsx não verifica `billingResult.success` da resposta | Código em 4.2.1 verifica `charge_timing` mas não `success` boolean (Gap 97). Erros genéricos são silenciados. FIX: adicionar `if (!billingResult.success && billingResult.error)` com toast ANTES de checar `charge_timing`. |
| 118 | Sem rollback de Stripe Invoice se DB insert falha (passos vi-vii) | Se Stripe Invoice é criada/finalizada (passos iv-v) mas `invoices`/`invoice_classes` insert falha, invoice Stripe existe pagável sem registro local. Webhook skip por "not found". FIX: wrap passos vi-vii em try/catch; em caso de falha, void Stripe Invoice antes de retornar erro. |
| 119 | Padrão de resposta inconsistente: `process-cancellation` (HTTP 500) vs `process-class-billing` (HTTP 200 + success:false) | Memory do projeto (`error-handling-user-friendly-messages`) recomenda HTTP 200 + success:false. FIX: documentar como inconsistência para alinhar na Fase 5 ou resolver imediatamente. |
| 120 | Agenda.tsx precisa importar namespace i18n `billing` para toasts | Chaves `chargeTiming.*` estão em `billing.json`. Agenda.tsx provavelmente usa `useTranslation('classes')`. FIX: usar `useTranslation(['classes', 'billing'])` ou prefixar chaves com `billing:`. |
| 121 | Resposta não detalha participantes skipados por mensalidade ativa | `processed`/`skipped` são agregados numéricos. Professor não sabe quem foi skipado nem porquê. FIX: adicionar `skipped_details?: Array<{ participant_id, student_name, reason }>` à resposta. |
| 122 | `process-class-billing` não valida preço e status do serviço server-side | Passo 3a-bis busca `class_services` sem verificar `is_active` e `price > 0`. Serviço desativado ou com preço R$0 causaria Invoice Stripe com valor 0 (rejeitado pelo Stripe). FIX: filtrar `WHERE is_active = true AND price > 0`; se inválido, skip com log. |
| 123 | `payment_settings.payment_method_types` não especificado na Invoice Stripe | Invoice aceita todos métodos da Connected Account, ignorando `enabled_payment_methods` do nosso DB. Decisão: INTENCIONAL — para Invoice Stripe, gerenciamento é nativo do Stripe. `enabled_payment_methods` aplica-se apenas ao fluxo Payment Intent. Documentar. |
| 124 | Sem limite de batch size — risco de timeout para muitas aulas | Edge functions têm timeout ~60s. 20+ aulas com múltiplas chamadas Stripe por aula podem exceder. FIX: validar `class_ids.length <= 20` com `success: false, error: 'Máximo de 20 aulas por chamada'`. Frontend deve particionar lotes maiores. |
| 125 | Seção 5.3 descreve modificações de webhook como incrementais mas são reescritas | "Inserir APÓS linha 333" sugere adição, mas o código proposto SUBSTITUI handlers inteiros (queries consolidadas, `.maybeSingle()`, null guards). Pode causar código duplicado se interpretado literalmente. FIX: refrasejar para "SUBSTITUIR handler completo (linhas X-Y)" em vez de "Inserir APÓS". |
