...
### Hierarquia de Geração Automática v2.5
...
## 6. Backend - Geração Condicional com Hierarquia v2.5
...
## 7. Backend - Validação, PIX Capability, Expiração e Limpeza v2.5
...
## 14. Arquivos a Modificar/Criar (v2.5 - ATUALIZADO)
...
| `supabase/functions/create-payment-intent-connect/index.ts` | **MODIFICAR** | **v2.5: Validar `enabled_payment_methods`, salvar `pix_expires_at`, limpar método anterior** + PIX capability + valor mínimo |
...
| `supabase/functions/webhook-stripe-connect/index.ts` | **MODIFICAR** | **v2.5: Limpeza COMPLETA de campos temporários ao pagar (CRÍTICO)** |
...
## 15. Checklist de Implementação (v2.5 - EXPANDIDO)
...
### Fase 3: Backend - Validação, Expiração e Limpeza (v2.5 - CRÍTICO)
- [ ] **v2.5**: `create-payment-intent-connect`: Query com `enabled_payment_methods` do business_profile
- [ ] **v2.5**: `create-payment-intent-connect`: Validar método habilitado antes de processar
...
- [ ] **v2.5**: `create-payment-intent-connect`: Salvar `pix_expires_at` ao criar PIX (CRÍTICO - FALTA IMPLEMENTAR)
- [ ] **v2.5**: `create-payment-intent-connect`: Limpar dados do método anterior ao gerar novo (CRÍTICO - FALTA IMPLEMENTAR)
- [ ] Testar validações
...
### Fase 5: Backend - Webhook (v2.5 - CRÍTICO)
- [ ] **v2.5**: `webhook-stripe-connect`: Limpeza COMPLETA de campos temporários ao pagar (CRÍTICO - FALTA IMPLEMENTAR)
- [ ] **v2.5**: Incluir `stripe_hosted_invoice_url` na limpeza além dos outros campos
...
- [ ] **v2.5**: Webhook limpa TODOS os campos temporários após pagamento (incluindo `stripe_hosted_invoice_url`)
- [ ] **v2.5**: `create-payment-intent-connect` salva `pix_expires_at` corretamente
- [ ] **v2.5**: `create-payment-intent-connect` valida `enabled_payment_methods` do business_profile
- [ ] **v2.5**: Ao gerar boleto, campos PIX anteriores são limpos
- [ ] **v2.5**: Ao gerar PIX, campos boleto anteriores são limpos
- [ ] **v2.5**: Ao iniciar Card, campos boleto E PIX são limpos

---

## 16. Estimativa de Tempo (v2.5 - ATUALIZADO)
...
| **v2.5**: Backend: Validação + PIX capability + `pix_expires_at` + limpeza método anterior | 2.5 horas |
| Backend: change-payment-method + config.toml (v2.2 atualizado) | 1.5 horas |
| **v2.5**: Backend: Webhook limpeza COMPLETA ao pagar | 45 min |
...
| i18n | 45 min |
| Testes (incluindo cenários v2.5) | 4.5 horas |
| **Total** | **~23 horas** |

---

## 17. Riscos e Mitigações (v2.5 - ATUALIZADO)
...
| **v2.5**: Webhook não limpa campos temporários → dados órfãos | Média | Alto | Limpeza COMPLETA obrigatória (incluindo `stripe_hosted_invoice_url`) |
| **v2.5**: `pix_expires_at` não salvo → PIX parece sempre válido | Alta | Alto | Verificar código de `create-payment-intent-connect` |
| **v2.5**: Método anterior não limpo → dados conflitantes | Média | Alto | Limpar campos do método anterior ao gerar novo |
