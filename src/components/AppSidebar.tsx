...
const getProfessorItems = (t: any) => [{
  ...
  title: t('navigation:sidebar.materials'),
  url: "/materiais",
  icon: FileText
}, {
...
const getAlunoItems = (t: any) => [{
  title: t('navigation:sidebar.studentPortal'),
  url: "/portal-do-aluno",
  icon: Users
}, {
  title: t('navigation:sidebar.myClasses'),
  url: "/aulas",
  icon: Calendar
}, {
  title: t('navigation:sidebar.myMaterials'),
  url: "/meus-materiais",
  icon: FileText
}, {
        ...
        <div className="flex h-full flex-col">
          <div className="flex h-16 items-center border-b px-4">
            <div className="flex items-center gap-2">
              ...
              {isOpen && <span className="text-lg font-semibold bg-gradient-primary bg-clip-text text-transparent">
                  TutorFlow
                </span>}
            </div>
          </div>
        ...
        {isAluno && !isProfessor}
                ...
                {items.map(item => <li key={item.title}>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div onClick={() => handleNavigation(item.url)} className={`flex items-center ${!isOpen ? 'justify-center w-12 h-10 px-3 py-2' : 'px-3 py-3'} rounded-lg min-h-[44px] w-full transition-all duration-200 ${item.title === t('navigation:sidebar.financial') && !(isProfessor ? hasFeature('financial_module') : hasTeacherFeature('financial_module')) ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'} ${isActive(item.url) ? 'bg-primary/20 text-primary font-semibold border border-primary/30 shadow-md backdrop-blur-sm' : 'text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground'}`}>
                          <item.icon className={`h-4 w-4 flex-shrink-0 text-primary`} />
                          {isOpen && <div className="flex items-center justify-between w-full ml-4">
                              <span>{item.title}</span>
                               {/* Show premium indicators */}
                               {item.title === t('navigation:sidebar.financial') && !(isProfessor ? hasFeature('financial_module') : hasTeacherFeature('financial_module')) && <span className="text-xs bg-warning/10 text-warning px-1.5 py-0.5 rounded">
          ...
          <div className={`border-t ${!isOpen ? 'p-3' : 'p-4'}`}>
            <div className={`flex items-center ${!isOpen ? "justify-center" : "justify-between"}`}>
              ...
              {!isOpen ? <Tooltip>
                  <TooltipTrigger asChild>
                    <Button variant="ghost" size="sm" onClick={handleSignOut} className="hover:bg-destructive hover:text-destructive-foreground">
                ...
                </Tooltip> : <Button variant="ghost" size="sm" onClick={handleSignOut} className="hover:bg-destructive hover:text-destructive-foreground" title={t('navigation:tooltips.logout')}>
                  <LogOut className="h-4 w-4" />
                </Button>}
            </div>
          </div>
