...
export default function MeusMateriais() {
  ...
  const filteredMaterials = materialAccess.filter(access => {
    ...
    return material.title.toLowerCase().includes(searchTerm.toLowerCase()) || material.description?.toLowerCase().includes(searchTerm.toLowerCase());
  });
  ...
  const handleDownload = async (material: Material) => {
    try {
      const {
        data,
        error
      } = await supabase.storage.from('teaching-materials').download(material.file_path);
      if (error) throw error;
  ...
  const handlePreview = async (material: Material) => {
    try {
      if (material.file_type.includes('pdf') || material.file_type.includes('image')) {
        const {
          data
        } = await supabase.storage.from('teaching-materials').getPublicUrl(material.file_path);
      ...
      <div className="space-y-6">
        ...
        {filteredMaterials.length === 0 ? <Card>
            <CardContent className="text-center py-8">
              ...
              <p className="text-muted-foreground">
                {searchTerm ? "Tente alterar os filtros de busca" : "Seu professor ainda não compartilhou materiais com você"}
              </p>
            </CardContent>
          </Card> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredMaterials.map(access => {
          const material = access.material;
          return <Card key={access.id} className="group hover:shadow-md transition-shadow">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {material.category && <Badge variant="secondary" className="mt-1" style={{
                  ...
                  <CardContent className="space-y-3">
                    {material.description && <p className="text-sm text-muted-foreground line-clamp-3">
        ...
        {filteredMaterials.length > 0 && <div className="text-center text-sm text-muted-foreground">
