...
-- Drop policies that depend on teacher_id column in profiles
DROP POLICY IF EXISTS "Professores podem ver seus alunos" ON public.profiles;
DROP POLICY IF EXISTS "Professores podem inserir alunos" ON public.profiles;
DROP POLICY IF EXISTS "Professores podem atualizar perfis de seus alunos" ON public.profiles;
DROP POLICY IF EXISTS "Professores podem excluir seus alunos" ON public.profiles;
DROP POLICY IF EXISTS "Students can access teacher subscription" ON public.user_subscriptions;

-- Remove teacher_id and billing_day columns from profiles
...
-- Recreate updated policies for profiles table using the new relationship table
CREATE POLICY "Professores podem ver seus alunos vinculados" 
ON public.profiles 
FOR SELECT 
USING (
  role = 'aluno' AND EXISTS (
...
FOR INSERT 
WITH CHECK (
  role = 'aluno' AND auth.uid() IN (
    SELECT p.id FROM profiles p WHERE p.id = auth.uid() AND p.role = 'professor'
  )
);

CREATE POLICY "Professores podem atualizar perfis de seus alunos vinculados" 
ON public.profiles 
FOR UPDATE 
USING (
  role = 'aluno' AND EXISTS (
...
CREATE POLICY "Professores podem excluir seus alunos vinculados" 
ON public.profiles 
FOR DELETE 
USING (
  role = 'aluno' AND EXISTS (
...
-- Recreate policy for user_subscriptions using the new relationship table
CREATE POLICY "Students can access teacher subscription via relationships" 
ON public.user_subscriptions 
FOR SELECT 
USING (
  user_id = auth.uid() OR user_id IN (
    SELECT tsr.teacher_id FROM public.teacher_student_relationships tsr
