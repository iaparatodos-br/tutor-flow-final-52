-- Adicionar sistema de estados e rollback à tabela existente

-- 1. Criar enum para status de processamento (se não existir)
DO $$ 
BEGIN
...
-- 2. Adicionar colunas de controle de processamento
ALTER TABLE public.processed_stripe_events 
ADD COLUMN processing_status processing_status_enum DEFAULT 'completed',
...
-- 3. Atualizar registros existentes com timestamps apropriados
...
-- 4. Função melhorada para iniciar processamento com controle de estado
        ...
        CASE v_existing_event.processing_status
            ...
            WHEN 'processing' THEN
                ...
                IF v_existing_event.processing_started_at < v_timeout_threshold THEN
                    -- Timeout detectado, marcar como timeout e permitir reprocessamento
                    UPDATE public.processed_stripe_events 
            ...
            WHEN 'failed' THEN
                -- Permitir retry se não excedeu limite
...
-- 5. Função para marcar evento como completado ou falho
...
-- 6. Função para cleanup de eventos órfãos
