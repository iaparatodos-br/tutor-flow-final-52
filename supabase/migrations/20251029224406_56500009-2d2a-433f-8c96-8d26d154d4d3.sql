-- FASE 4 & 5 (CORRIGIDA): Drop e recriar function, depois remover coluna

-- 1. DROP da função antiga primeiro
DROP FUNCTION IF EXISTS public.get_classes_with_participants(uuid, timestamp with time zone, timestamp with time zone);

-- 2. CRIAR função atualizada (student_id sempre NULL)
...
-- 3. REMOVER COLUNA student_id CASCADE
ALTER TABLE classes DROP COLUMN IF EXISTS student_id CASCADE;

-- 4. REMOVER COLUNA DE BACKUP
